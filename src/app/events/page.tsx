...
interface WeatherData {
  hourly: {
    ...
    apparent_temperature: number[]; // Add apparent temperature
    precipitation: number[];
    windspeed_10m: number[];
    cloudcover: number[];
    uv_index: number[]; // Add UV index
  };
  hourly_units?: {
    temperature_2m: string;
    apparent_temperature: string; // Add unit
    precipitation: string;
    windspeed_10m: string;
    cloudcover: string;
    uv_index: string; // Add unit
  };
  daily: {
    time: string[];
    sunrise: string[];
    sunset: string[];
    precipitation_sum: number[]; // Add daily precipitation sum
    temperature_2m_max: number[]; // Add max temperature
    temperature_2m_min: number[]; // Add min temperature
  };
}
...
export default function Events(): JSX.Element {
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      {/* Weather Widget with Chart Background and Day Cycle Gradient */}
      <div
        className="p-4 rounded-lg shadow mb-6 relative overflow-hidden"
        style={{
          background: `${dayCycleGradient}, black`,
          minHeight: '250px'
        }}
      >
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="absolute inset-0 opacity-60 z-100" style={{ height: '100%' }}>
                  ...
                  {weatherData && weatherData.daily && weatherData.daily.sunrise && (
                    <ReferenceLine
                      ...
                      label={{
                        ...
                        fontSize: 16,
                        dy: 15, // Offset vertically to move down from the top edge
                        dx: 10 // Offset horizontally
                      }}
                    />
                  )}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 relative z-10 mt-3">
              {weatherData && (
                <>
                  <div className="bg-white/40 dark:bg-black/20 bg-opacity-20 backdrop-blur-sm p-3 rounded">
