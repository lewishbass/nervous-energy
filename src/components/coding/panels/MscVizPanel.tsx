	...
	function MscVizPanel({ isCompact = false }, ref) {
		...
		return (
			<div className="flex-1 overflow-hidden flex flex-col min-h-0 h-full">
				...
				<div ref={containerRef} className="flex-grow bg2 tc1 overflow-hidden relative">
					{/* paths canvas: persistent, incrementally drawn (full repaint on resize) */}
					<canvas
						ref={pathsCanvasRef}
						className="absolute inset-0 w-full h-full"
					/>
					{/* overlay canvas: robot indicators + heading, redrawn on each action */}
					<canvas
