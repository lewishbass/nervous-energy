...
function summaryToInfo(summary: Record<string, VariableSummary>): Record<string, VariableInfo> {
	...
	for (const [key, v] of Object.entries(summary)) {
		result[key] = {
			...
			callable: v.callable,
			expanded: v.expanded,
			children: v.children ? summaryToInfo(v.children) : undefined,
		};
	}
	return result;
}

export default function AsyncPyIde({
	initialCode = '',
	...
	// ---- stable task id for this IDE instance ----
	const taskId = useMemo(
		() => initialDocumentId ?? `ide-${Math.random().toString(36).substr(2, 9)}`,
	...
	const [lastArgs, setLastArgs] = useState<any>(null);

	const [preCode, setPreCode] = useState<string>(preCodeProp ?? '');
	...
	useEffect(() => {
		const unsub = pyodidePool.subscribe(taskId, (entry: OutputEntry) => {
			...
			if (responseType === 'stdout') {
				...
				setTerminalLines(prev => [...prev, { text, type: 'stdout' }]);
				...
				// fire callback
				if (onCodeEndCallback) {
					const flatVars = vars ? summaryToInfo(buildVariableTree(vars)) : {};
					onCodeEndCallback(
						codeRef.current,
						null,
						error,
						flatVars,
						stdoutRef.current || null,
					);
				}
		...
		return unsub;
	}, [taskId, highlightLine, buildVariableTree, onCodeEndCallback]);
	...
	const ensureKernel = useCallback(async () => {
		...
		try {
			...
			if (unsureKernelLock.current) {
				console.warn("Already checking for kernel worker, skipping concurrent check");
				return null;
			}
			unsureKernelLock.current = true;
			const worker = await pyodidePool.allocateWorker(taskId, 'kernel', 'IDE Kernel');
			unsureKernelLock.current = false;
			if (!worker) {
	...
	const handleRun = useCallback(async () => {
		...
		if (onCodeStartCallback) {
			onCodeStartCallback(codeRef.current, null);
		}
	...
	}, [ensureKernel, documentName, persistentExec, onCodeStartCallback]);
	...
	}, [running, ensureKernel, documentName, persistentExec, sendFlowControl, onCodeStartCallback]);
	const handleStop = useCallback(() => sendFlowControl('stop'), [sendFlowControl]);

	const handleRestart = useCallback(async () => {
		const workerId = await ensureKernel();
		if (!workerId) return;
		pyodidePool.messageWorker(workerId, { type: 'flow_control', flowAction: 'reset' });
		setRunning(false);
		setPaused(false);
		setFrames([]);
		setTerminalLines([]);
	...
	// ---- cached code ----
	const handleLoadCachedCode = useCallback(() => {
		if (cachedCode != null) {
			codeRef.current = cachedCode;
			monacoRef.current?.setValue(cachedCode);
		}
	}, [cachedCode]);
	...
	return (
		<div className="relative w-full overflow-hidden flex flex-col h-full">
			...
			<ToolbarPanel
				...
				onLoadCachedCode={handleLoadCachedCode}
			/>

			{/* ---- Body ---- */}
			<DraggableDivider
				direction="horizontal"
				initialPosition={initialHDivider}
				edgeSize={[15, 15]}
				collapseEnd={true}
				color="purple"
